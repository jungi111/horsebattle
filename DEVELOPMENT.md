...
`adapter: 'local'` 상태에서는 같은 브라우저의 탭끼리만 동기화된다. 로그인 세션은 탭 단위라 탭마다 다른 회원으로 들어갈 수 있다. 계정이 자동으로 만들어지지는 않으므로, 브라우저 콘솔에서 `auth.register('아이디', '비밀번호')` 를 한 번 호출해 로컬 계정을 만든 뒤 로그인한다.
